...
class CustomerOrder:
    ...
    def _invoice_number(self):
        self.invoice_number = "INV-" + self.order_id.replace('_', '-')
    
    def format_invoice_number(self):
        return "INV-" + self.invoice_number[4:].replace('-', '/')
    ...
    def print_invoice(self, card_type):
        ...
        print(textwrap.dedent(f"""\n
            ...
            {f"No: {self.format_invoice_number()}".rjust(self.INVOICE_WIDTH)}
            {bar}
            {self._format_order()}
            {bar}
            {self._format_value('Sub Total', self.amount)}
            {self._format_value(f'Sales Tax ({self.SALES_TAX * 100}%)', self.get_sales_tax())}
...
def log_order(order):
    ...
    filename = pathlib.Path(__file__).parent / '_invoices' / order.invoice_number
    with open(filename, 'w') as file:
        file.write(order._format_order())
